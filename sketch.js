...
function draw(){
    ...
    if (mouseReleased){
        ...
        for (var i = 1; i < currentPathA.length-1; i++) {
            ...
            stroke(currentPathA[i].rStroke,12,14);
            fill(currentPathA[i].colorfill);
              //rect(currentPathA[i].x, currentPathA[i].y,7,7);
                line(currentPathA[i-1].x,currentPathA[i-1].y,currentPathA[i].x,currentPathA[i].y);
            //line(currentPathA[i].x, currentPathA[i].y,7,7);   
        }
    }
